      ...
      <div className="mx-auto grid max-w-6xl gap-12 px-5 pb-16 pt-14 sm:px-6 sm:pt-20 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:pb-24 lg:pt-24">
        <div>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3">
            <a
              href="#waitlist"
              className="brand-gradient rounded-full px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-900/15 transition-transform hover:scale-[1.02]"
            >
              {t("ctaPrimary")}
            </a>
            <a
              href="#how"
              className="rounded-full border border-brand-border px-6 py-3 text-sm font-semibold text-brand-text transition-colors hover:bg-brand-muted-bg"
            >
              {t("ctaSecondary")}
            </a>
          </div>
          ...
          <dl className="mt-10 grid max-w-lg grid-cols-3 gap-4 border-t border-brand-border pt-6">
            {badges.map((badge) => (
              <div key={badge.label}>
                ...
                <dd className="text-2xl font-semibold text-brand-text">{badge.value}</dd>
